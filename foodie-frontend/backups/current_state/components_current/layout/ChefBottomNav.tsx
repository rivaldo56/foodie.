...
import { LayoutDashboard, Compass, ShoppingBag, MessageCircle, UserRound } from 'lucide-react';

const CHEF_LINKS = [
  { href: '/chef/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/chef/discover', label: 'Discover', icon: Compass },
  { href: '/chef/bookings', label: 'Bookings', icon: ShoppingBag },
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex justify-center bg-black/50 pb-4 pt-2 backdrop-blur">
      <div className="flex w-full max-w-xl items-center justify-between gap-2 rounded-full border border-white/10 bg-black/70 px-4 py-2 text-xs font-medium text-white/70">
        {CHEF_LINKS.map(({ href, label, icon: Icon }) => {
