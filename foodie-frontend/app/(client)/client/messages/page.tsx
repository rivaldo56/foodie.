        ...
        setConversations((prev) =>
          prev.map(c =>
            c.id === activeConversationId
              ? { ...c, last_message: response.data!, updated_at: new Date().toISOString() }
              : c
          ...
          {isLoadingConversations ? (
            ...
            <div className="divide-y divide-white/5">
              {conversations.map((conv) => {
                const otherUser = conv.other_user || (conv as any).chef || (conv as any).client; // Handle different API structures
        ...
        {activeConversationId ? (
            ...
            <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-surface/50 backdrop-blur">
              <div className="flex items-center gap-3">
                ...
                <div className="h-8 w-8 rounded-full bg-accent/20 flex items-center justify-center text-accent font-semibold">
                  {(activeConversation?.other_user?.full_name || 'U').charAt(0)}
                </div>
                <div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-muted p-8">
            ...
            <p className="max-w-xs text-center">
              Choose a chat from the sidebar or start a new conversation from a chef's profile.
            </p>
          </div>
        )}
      </div>
